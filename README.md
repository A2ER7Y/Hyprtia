...
Hyprtia is the StratOS edition of [Noctalia v5](https://github.com/noctalia-dev/noctalia-shell), pinned to
`v5.0.0-beta2`. It keeps Noctalia's binary, IPC, plugin, and configuration compatibility while adding an Arch package,
StratOS defaults, a `hyprtia` launcher, and Hyprland session integration.
...
Start Hyprtia from the StratOS Hyprland session by adding this line after the
`dbus-update-activation-environment` command in `~/.config/hypr/autostart.conf`:

```ini
exec-once = hyprtia --daemon
```

Alternatively, start it as a user service after the Hyprland environment has been imported:

```sh
systemctl --user enable --now hyprtia.service
```
...
pinned = ["firefox.desktop", "Notion.desktop"]
```

Pinned entries use the same XDG desktop-entry discovery as the launcher and dock. This includes web launchers generated by
...
```ini
bind = SUPER, SPACE, exec, hyprtia msg panel-toggle launcher
...
bindle = , XF86MonBrightnessUp, exec, hyprtia msg brightness-up
bindle = , XF86MonBrightnessDown, exec, hyprtia msg brightness-down
```
